...
public final class FootballCommand implements CommandExecutor {
    ...
    private boolean onFootballStadiumList(
        ...
        sendPrefixMessage(commandSender, "Football-Stadiums: §e" + footballStadiums.size());
        sendPrefixMessage(commandSender, "Football-Stadium-List:");
        footballStadiums.forEach(footballStadium -> {
    ...
    private boolean onFootballStadiumCreate(
        ...
        if (arguments.length == 3) {
            ...
            footballStadiumRepository.find(footballStadiumName).ifPresentOrElse(footballStadium -> {
                sendPrefixMessage(commandSender, "There is already a stadium with that name.");
            }, () -> {
                ...
                sendPrefixMessage(commandSender, "You successfully added a new stadium.");
            });
        } else {
    ...
    private boolean onFootballStadiumGoal(
        ...
        if (arguments.length == 6) {
            ...
            if (patternCompiler.matcher(widthString).matches() &&
                    patternCompiler.matcher(heightString).matches()
            ) {
                var width = Integer.parseInt(widthString);
                var height = Integer.parseInt(heightString);
                footballStadiumRepository.find(footballStadiumName).ifPresentOrElse(footballStadium -> {
                    try {
                        var direction = DefaultFootballGoal.FootballGoalDirection.valueOf(directionString.toUpperCase());
                        var player = (Player) commandSender;
                        var playerLocation = player.getLocation();
                        var footballGoal = DefaultFootballGoal.of(playerLocation, width, height, direction);
                        footballStadium.addFootballGoal(footballGoal);
                        sendPrefixMessage(commandSender, "You successfully added a new goal to the stadium.");
                    } catch (Exception exception) {
                        sendPrefixMessage(commandSender, "Can't find the direction " + directionString + ".");
                    }
                }, () -> {
                    sendPrefixMessage(commandSender, "Can't find any stadium with that name.");
                });
            } else {
            ...
            sendSyntaxCorrection(commandSender, "/football stadium goal <stadium name> <width> <height> <direction>");
            return false;
        }
        return true;
    }
    ...
    private boolean onFootballStadiumInfo(
        ...
        if (arguments.length == 3) {
            var footballStadiumName = arguments[2];
            footballStadiumRepository.find(footballStadiumName).ifPresentOrElse(footballStadium -> {
                sendPrefixMessage(commandSender, "Football-Stadium-" + footballStadiumName + ":");
                ...
                sendPrefixMessage(commandSender, "      - X: " + footballStadiumCenterLocation.getX());
                sendPrefixMessage(commandSender, "      - Y: " + footballStadiumCenterLocation.getY());
                sendPrefixMessage(commandSender, "      - Z: " + footballStadiumCenterLocation.getZ());
                sendPrefixMessage(commandSender, "  - Goals: " + footballStadium.footballGoals().length);
            }, () -> {
                sendPrefixMessage(commandSender, "Can't find any stadium with that name.");
            });
        } else {
    ...
    private boolean onFootballStadiumDelete(
        ...
        if (arguments.length == 3) {
            ...
            footballStadiumRepository.find(footballStadiumName).ifPresentOrElse(footballStadium -> {
                ...
                sendPrefixMessage(commandSender, "You successfully removed the stadium.");
            }, () -> {
                sendPrefixMessage(commandSender, "Can't find any stadium with that name.");
            });
        } else {
    ...
    private boolean onFootballStadiumOverview(
        ...
        sendPrefixMessage(commandSender, "  - /football stadium goal <stadium name> <width> <height> <direction>");
    ...
    private boolean onFootballGameList(
        ...
        sendPrefixMessage(commandSender, "Football-Games: §e" + footballGames.size());
        sendPrefixMessage(commandSender, "Football-Game-List:");
        footballGames.forEach(footballGame -> {
            sendPrefixMessage(commandSender, "  - " + footballGame.footballStadium().name());
        });
        return true;
    }
    ...
    private boolean onFootballGameCreate(
        ...
        if (arguments.length == 5) {
            ...
            footballStadiumRepository.find(footballStadiumName).ifPresentOrElse(footballStadium -> {
                if (footballGameRegistry.findGameByStadium(footballStadium).isEmpty()) {
                    var footballGoals = footballStadium.footballGoals();
                    var firstTeamOptional = parseFootballTeam(firstTeamPlayers, footballGoals[0]);
                    var secondTeamOptional = parseFootballTeam(secondTeamPlayers, footballGoals[1]);
                    if (firstTeamOptional.isEmpty() || secondTeamOptional.isEmpty()) {
                        sendPrefixMessage(commandSender, "One of the provided players is not online.");
                    }
                    var firstTeam = firstTeamOptional.get();
                    var secondTeam = secondTeamOptional.get();
                    var football = defaultFootballFactory
                            .createDefaultFootball(footballStadium.stadiumCenter());
                    ...
                    sendPrefixMessage(commandSender, "You successfully created a new football game at " +
                            "the stadium " + footballStadiumName + ".");
                } else {
                    sendPrefixMessage(commandSender, "There is already a football game running to that stadium.");
                }
            }, () -> {
                sendPrefixMessage(commandSender, "Can't find any stadium with that name.");
            });
        } else {
            sendSyntaxCorrection(commandSender, "/football game create <stadium> <players of the first team> " +
                    "<players of the second team> (Players-Format: DerCoder;notch;...)");
            return false;
        }
        return true;
    }
    ...
    private Optional<FootballTeam> parseFootballTeam(
        ...
        var footballTeam = FootballTeam.of(
                players, footballGoal, 0
        );
    ...
    private boolean onFootballGameInfo(
        ...
        if (arguments.length == 3) {
            var footballStadiumName = arguments[2];
            footballStadiumRepository.find(footballStadiumName).ifPresentOrElse(footballStadium -> {
                footballGameRegistry.findGameByStadium(footballStadium).ifPresentOrElse(footballGame -> {
                    sendPrefixMessage(commandSender, "Football-Game-" + footballStadiumName + ":");
                    sendPrefixMessage(commandSender, "  - Stadium: " + footballStadiumName);
                    sendPrefixMessage(commandSender, "  - Teams: ");
                    var footballTeams = footballGame.footballTeams();
                    displayFootballTeam(commandSender, footballGame, footballTeams[0], "First");
                    displayFootballTeam(commandSender, footballGame, footballTeams[1], "Second");
                }, () -> {
                    sendPrefixMessage(commandSender, "Can't find any football game at that stadium.");
                });
            }, () -> {
                sendPrefixMessage(commandSender, "Can't find any stadium with that name.");
            });
    ...
    private void displayFootballTeam(
        ...
        footballGame.findTeamPlayers(footballTeam).forEach(player -> {
    ...
    private boolean onFootballGameStop(
        ...
        if (arguments.length == 3) {
            ...
            footballStadiumRepository.find(footballStadiumName).ifPresentOrElse(footballStadium -> {
                footballGameRegistry.findGameByStadium(footballStadium).ifPresentOrElse(footballGame -> {
                    footballGame.close();
                    sendPrefixMessage(commandSender, "You successful stopped the football game.");
                }, () -> {
                    sendPrefixMessage(commandSender, "Can't find any football game at that stadium.");
                });
            }, () -> {
    ...
    private boolean onFootballGameOverview(
        ...
        sendPrefixMessage(commandSender, "Football-Game:");
        sendPrefixMessage(commandSender, "  - /football game list");
        sendPrefixMessage(commandSender, "  - /football game create <stadium> <players of the first team> " +
                "<players of the second team> (Players-Format: DerCoder;notch;...)");
    ...
    private void sendPrefixMessage(
            CommandSender commandSender,
            String message
    ) {
