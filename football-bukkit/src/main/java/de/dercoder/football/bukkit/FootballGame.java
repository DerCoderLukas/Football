...
public final class FootballGame {
  ...
  private void scoreGoal(DefaultFootballGoal footballGoal) {
    ...
    shooter.shootAGoal();
    footballMatch.findTeamOfGoal(footballGoal).ifPresent(FootballTeam::shootAGoal);
    sendGoalTitles();
    football.reset();
    resetTeams();
  }
  ...
  private void teleportTeam(FootballTeam footballTeam) {
    var footballGoalLocation = ((DefaultFootballGoal) footballTeam.footballGoal()).goalLocation();
  ...
  private void sendIntermediateTitles() {
    ...
    var intermediateScoreMessage =
        new TextComponent(
            footballConfiguration.parseIntermediateScoreMessage(
                teams[0].goals(), teams[1].goals()));
    var players = findAllPlayers().collect(Collectors.toUnmodifiableList());
    for (var player : players) {
      player.spigot().sendMessage(ChatMessageType.ACTION_BAR, intermediateScoreMessage);
    }
  }
  ...
  private void sendGoalTitles() {
    ...
    var scoreMainMessage =
        footballConfiguration.parseScoreMainMessage(scorerName, teams[0].goals(), teams[1].goals());
    var scoreSecondaryMessage =
        footballConfiguration.parseScoreSecondaryMessage(
            scorerName, teams[0].goals(), teams[1].goals());
    var players = findAllPlayers().collect(Collectors.toUnmodifiableList());
    for (var player : players) {
  ...
  public Stream<Player> findTeamPlayers(FootballTeam footballTeam) {
    ...
    return footballTeam.players().stream()
        .map(footballPlayer -> Bukkit.getPlayer(footballPlayer.id()))
        .filter(Objects::nonNull);
  }

  public Stream<Player> findAllPlayers() {
    return footballMatch.players().stream()
        .map(footballPlayerSession -> Bukkit.getPlayer(footballPlayerSession.player().id()))
        .filter(Objects::nonNull);
  }
