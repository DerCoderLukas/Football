...
public final class FootballGame {
    ...
    public void start() {
        footballGameTask = Bukkit.getScheduler().scheduleAsyncRepeatingTask(
    ...
    private void teleportTeam(FootballTeam footballTeam) {
        ...
        findTeamPlayers(footballTeam).forEach(player -> {
    ...
    private void sendGoalTitles() {
        var teams = footballMatch.teams().get();
        var scorerName = Bukkit
                .getPlayer(football().shooter().id())
                .getName();
    ...
    public Stream<Player> findTeamPlayers(
        ...
        return footballTeam.players()
                .stream()
                .map(footballPlayer -> {
                    return Bukkit.getPlayer(footballPlayer.id());
                })
    ...
    public Stream<Player> findAllPlayers() {
        return footballMatch.players()
                .stream()
                .map(footballPlayerSession -> {
                    var footballPlayer = footballPlayerSession.player();
                    return Bukkit.getPlayer(footballPlayer.id());
                })
                .filter(Objects::nonNull);
    }
