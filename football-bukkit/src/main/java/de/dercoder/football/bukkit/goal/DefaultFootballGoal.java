...
public final class DefaultFootballGoal implements FootballGoal {
  ...
  public enum FootballGoalDirection {
  ...
  public boolean isInGoal(DefaultFootball football) {
    ...
    if (footballLocation.getY() < goalLocationHeight || footballLocation.getY() > goalLocationHeight + height) {
      return false;
    }
    var goalWidthHalf = ((double) width) / 2.0;
    switch (direction) {
      case NORTH:
        return (footballLocation.getZ() > goalLocation.getZ() && footballLocation
          .getX() > goalLocation.getX() - goalWidthHalf && footballLocation.getX() < goalLocation
          .getX() + goalWidthHalf);
      case EAST:
        return (footballLocation.getX() < goalLocation.getX() && footballLocation
          .getZ() > goalLocation.getZ() - goalWidthHalf && footballLocation.getZ() < goalLocation
          .getZ() + goalWidthHalf);
      case SOUTH:
        return (footballLocation.getZ() < goalLocation.getZ() && footballLocation
          .getX() > goalLocation.getX() - goalWidthHalf && footballLocation.getX() < goalLocation
